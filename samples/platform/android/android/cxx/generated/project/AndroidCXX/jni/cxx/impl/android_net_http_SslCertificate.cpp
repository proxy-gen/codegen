...
{
	LOGV("android_net_http_SslCertificate::android_net_http_SslCertificate(const android_net_http_SslCertificate& cc) invoked");
...
{
	LOGV("android_net_http_SslCertificate::android_net_http_SslCertificate(void * proxy) invoked");
...
{
	LOGV("android_net_http_SslCertificate::~android_net_http_SslCertificate() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
