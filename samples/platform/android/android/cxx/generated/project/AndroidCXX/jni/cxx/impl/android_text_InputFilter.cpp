...
#define LOG_TAG "android_text_InputFilter"
...
template void convert_proxy<android_text_Spanned>(long& java_value, long& cxx_value, const CXXTypeHierarchy cxx_type_hierarchy, const converter_t& converter_type, std::stack<long>& converter_stack);
...
java_lang_CharSequence *  android_text_InputFilter::filter(java_lang_CharSequence& arg0,int& arg1,android_text_Spanned& arg2)
{
	...
	const char *methodSignature = "(Ljava/lang/CharSequence;ILandroid/text/Spanned;)Ljava/lang/CharSequence;";
	const char *className = "android_text_InputFilter";
	...
	LOGV("android_text_InputFilter jni address %d", javaObject);
	...
	jobject jarg2;
	{
		...
		cxx_type_hierarchy_stack.push(cxx_type_hierarchy);
		{
			CXXTypeHierarchy cxx_type_hierarchy = cxx_type_hierarchy_stack.top();
			cxx_type_hierarchy_stack.pop();
			cxx_type_hierarchy.type_name = std::string("android.text.Spanned");
		}
		std::stack<long> converter_stack;
		converter_t converter_type = (converter_t) CONVERT_TO_JAVA;
		convert_proxy<android_text_Spanned>(java_value,cxx_value,cxx_type_hierarchy,converter_type,converter_stack);
