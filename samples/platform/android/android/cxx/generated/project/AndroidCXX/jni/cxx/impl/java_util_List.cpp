...
template void convert_proxy<java_util_ListIterator>(long& java_value, long& cxx_value, const CXXTypeHierarchy cxx_type_hierarchy, const converter_t& converter_type, std::stack<long>& converter_stack);
...
java_util_ListIterator *  java_util_List::listIterator(int& arg0)
{
	...
	long java_value = convert_jni_java_lang_Object_to_java(jni_result);
	{
		...
		converter_t converter_type = (converter_t) CONVERT_TO_CXX;
		convert_proxy<java_util_ListIterator>(java_value,cxx_value,cxx_type_hierarchy,converter_type,converter_stack);
	}
