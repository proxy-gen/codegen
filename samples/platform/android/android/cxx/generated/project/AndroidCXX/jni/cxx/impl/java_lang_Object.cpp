...
{
	LOGV("java_lang_Object::java_lang_Object(const java_lang_Object& cc) invoked");
...
{
	LOGV("java_lang_Object::java_lang_Object(void * proxy) invoked");
...
{
	LOGV("java_lang_Object::~java_lang_Object() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
