...
{
	LOGV("android_widget_SimpleAdapter::android_widget_SimpleAdapter(const android_widget_SimpleAdapter& cc) invoked");
...
{
	LOGV("android_widget_SimpleAdapter::android_widget_SimpleAdapter(void * proxy) invoked");
...
{
	LOGV("android_widget_SimpleAdapter::~android_widget_SimpleAdapter() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
