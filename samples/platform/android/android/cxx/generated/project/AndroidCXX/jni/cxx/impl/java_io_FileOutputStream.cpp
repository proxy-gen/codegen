...
{
	LOGV("java_io_FileOutputStream::java_io_FileOutputStream(const java_io_FileOutputStream& cc) invoked");
...
{
	LOGV("java_io_FileOutputStream::java_io_FileOutputStream(void * proxy) invoked");
...
{
	LOGV("java_io_FileOutputStream::~java_io_FileOutputStream() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
