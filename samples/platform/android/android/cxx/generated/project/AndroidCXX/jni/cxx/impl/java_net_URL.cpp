...
template void convert_proxy<java_net_Proxy>(long& java_value, long& cxx_value, const CXXTypeHierarchy cxx_type_hierarchy, const converter_t& converter_type, std::stack<long>& converter_stack);

template void convert_proxy<java_net_URLConnection>(long& java_value, long& cxx_value, const CXXTypeHierarchy cxx_type_hierarchy, const converter_t& converter_type, std::stack<long>& converter_stack);

template void convert_proxy<java_net_URLStreamHandlerFactory>(long& java_value, long& cxx_value, const CXXTypeHierarchy cxx_type_hierarchy, const converter_t& converter_type, std::stack<long>& converter_stack);

// Default Instance Constructors
java_net_URL::java_net_URL(const java_net_URL& cc)
{
	LOGV("java_net_URL::java_net_URL(const java_net_URL& cc) invoked");
...
{
	LOGV("java_net_URL::java_net_URL(void * proxy) invoked");
...
{
	LOGV("java_net_URL::~java_net_URL() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
...
java_net_URLConnection *  java_net_URL::openConnection(java_net_Proxy& arg0)
{
	...
	jobject jarg0;
	{
		...
		std::stack<long> converter_stack;
		converter_t converter_type = (converter_t) CONVERT_TO_JAVA;
		convert_proxy<java_net_Proxy>(java_value,cxx_value,cxx_type_hierarchy,converter_type,converter_stack);
	...
	long java_value = convert_jni_java_lang_Object_to_java(jni_result);
	{
		...
		std::stack<long> converter_stack;
		converter_t converter_type = (converter_t) CONVERT_TO_CXX;
		convert_proxy<java_net_URLConnection>(java_value,cxx_value,cxx_type_hierarchy,converter_type,converter_stack);
	}
...
java_lang_Object *  java_net_URL::getContent(std::vector<long>& arg0)
{
...
void java_net_URL::setURLStreamHandlerFactory(java_net_URLStreamHandlerFactory& arg0)
{
	...
	jobject jarg0;
	{
		...
		std::stack<long> converter_stack;
		converter_t converter_type = (converter_t) CONVERT_TO_JAVA;
		convert_proxy<java_net_URLStreamHandlerFactory>(java_value,cxx_value,cxx_type_hierarchy,converter_type,converter_stack);
