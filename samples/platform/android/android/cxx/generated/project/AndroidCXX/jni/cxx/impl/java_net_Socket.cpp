...
{
	LOGV("java_net_Socket::java_net_Socket(const java_net_Socket& cc) invoked");
...
{
	LOGV("java_net_Socket::java_net_Socket(void * proxy) invoked");
...
{
	LOGV("java_net_Socket::~java_net_Socket() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
