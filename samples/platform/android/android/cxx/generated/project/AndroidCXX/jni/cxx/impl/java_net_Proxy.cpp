...
{
	LOGV("java_net_Proxy::java_net_Proxy(const java_net_Proxy& cc) invoked");
...
{
	LOGV("java_net_Proxy::java_net_Proxy(void * proxy) invoked");
...
{
	LOGV("java_net_Proxy::~java_net_Proxy() invoked");
	CXXContext *ctx = CXXContext::sharedInstance();
	long address = (long) this;
	jobject proxiedComponent = ctx->findProxyComponent(address);
	if (proxiedComponent != 0)
...
java_net_Proxy_Type *  java_net_Proxy::type()
{
	...
	long java_value = convert_jni_java_lang_Object_to_java(jni_result);
	{
		...
		convert_proxy<java_net_Proxy_Type>(java_value,cxx_value,cxx_type_hierarchy,converter_type,converter_stack);
	}
